...
public class ScrollableTestUsingUpdatableResultSet {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(rs!=null) {
				...
				//Thread.sleep(10000);
				while(rs.next()) {
				//	rs.refreshRow();  //not supported in INSENSITIVE
					System.out.println(rs.getRow()+"    "+rs.getInt(1)+"     "+rs.getString(2)+"      "+rs.getString(3)+"          "+rs.getString(4));
				}
				
				rs.moveToInsertRow();
				rs.updateInt(1,169);
				rs.updateString(2,"vaman");
				rs.updateString(3, "guntur");
				rs.updateString(4,"JavaScript");
				rs.insertRow();
				
				rs.absolute(9);
				rs.updateString(2,"Amir");
				rs.updateRow();
